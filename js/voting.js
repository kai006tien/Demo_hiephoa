...
const Voting = {
  ...
  renderVoteListAdmin() {
    ...
    if (activeVotes.length > 0) {
      html += `<h3 class="vote-group-title"><span class="pulse-dot"></span> Biểu quyết đang diễn ra (${activeVotes.length})</h3>`;
    ...
    if (closedVotes.length > 0) {
      html += `<h3 class="vote-group-title text-muted mt-8">✓ Biểu quyết đã kết thúc (${closedVotes.length})</h3>`;
